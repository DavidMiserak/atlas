...
void main() {
  ...
  Future<void> insertWarmupSet(int sessionExerciseId, int setNumber) async {
    final db = await getDatabase();
    await db.insert(tableSessionSets, {
      colSessionSetSessionExerciseId: sessionExerciseId,
      colSessionSetNumber: setNumber,
  ...
  group('ProgressionService', () {
    ...
    test(
      '1. All working sets completed → progression earned, correct new 1RM',
      () async {
        await loadSeedData();
        ...
        await insertWorkingSet(
          seId,
          setNumber: 1,
          repsCompleted: repsMin,
          weightLifted: 195.0,
        );
        await insertWorkingSet(
          seId,
    ...
    test(
      '3. Set with null repsCompleted → treated as 0, no progression',
      () async {
        await loadSeedData();
        final info = await slotInfo('Squat Variant');
        final slotId = info['slotId'] as int;
    ...
    test('8. Deload session → returns empty list, no 1RM updates', () async {
      ...
      final oneRmBefore = await OneRmRepository().getCurrentOneRm(variantId);
      ...
      final oneRmAfter = await OneRmRepository().getCurrentOneRm(variantId);
      expect(oneRmAfter, equals(oneRmBefore));
    });

    test('8b. Demo session → returns empty list, no 1RM updates', () async {
      await loadSeedData();
      final info = await slotInfo('Squat Variant');
      final slotId = info['slotId'] as int;
      final variantId = info['variantId'] as int;
      ...
      final sessionId = await insertSession(isDemo: true);
      final seId = await insertSessionExercise(sessionId, slotId, variantId);
      for (var i = 1; i <= 3; i++) {
        await insertWorkingSet(
          seId,
    ...
    test('9. Set template with null repsTargetMin → exercise skipped', () async {
      await loadSeedData();
      final db = await getDatabase();
    ...
    test(
      '10. All reps met but weightLifted is null → exercise skipped',
      () async {
        await loadSeedData();
        final info = await slotInfo('Squat Variant');
        final slotId = info['slotId'] as int;
    ...
    test(
      '12. 7-day cooldown: session within 7 days of last 1RM → no progression',
      () async {
        await loadSeedData();
        final info = await slotInfo('Squat Variant');
        final slotId = info['slotId'] as int;
    ...
    test(
      '13. 7-day cooldown: session 8+ days after last 1RM → progression fires',
      () async {
        await loadSeedData();
        final info = await slotInfo('Squat Variant');
        final slotId = info['slotId'] as int;
    ...
    test(
      '14. Epley formula: qualifying sets compute 1RM via reps, progression fires',
      () async {
        await loadSeedData();
        final info = await slotInfo('Horizontal Push');
        final slotId = info['slotId'] as int;
    ...
    test(
      '15. Rep-based formula with null repsCompleted → hard skip, no progression',
      () async {
        await loadSeedData();
        final info = await slotInfo('Horizontal Push');
        final slotId = info['slotId'] as int;
    ...
    test(
      '11. RPE target outside rpeToPercent range → exercise skipped',
      () async {
        await loadSeedData();
        final db = await getDatabase();
